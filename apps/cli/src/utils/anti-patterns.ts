...
import type { DetectedStack, ProjectProfile } from './stack-detect.js';
...
/**
 * Build anti-patterns.md content from a ProjectProfile.
 * Uses the AI-detected anti-patterns from the profile, enriched with stack context.
 * This replaces the tag-filter approach with project-specific patterns derived from
 * actual workspace analysis.
 */
export function buildAntiPatternsFromProfile(profile: ProjectProfile): string {
  const stackLabel =
    profile.stack.length > 0 ? profile.stack.join(', ') : 'detected stack';

  const header = [
    `# Anti-Patterns — Project Rules`,
    ``,
    `Generated by \`nitro-fueled init\` from workspace analysis. Stack: **${stackLabel}**.`,
    ``,
    `Check these BEFORE submitting work.`,
    ``,
  ].join('\n');
  ...
  if (profile.antiPatterns.length === 0) {
    return header + '_No project-specific anti-patterns detected. Check anti-patterns-master.md._\n';
  }

  const rules = profile.antiPatterns
    .map((pattern, i) => `## Rule ${i + 1}: Avoid\n\n${pattern}`)
    .join('\n\n');
...
/**
 * Generates anti-patterns.md from a ProjectProfile (AI-analysis path).
 * Uses profile.antiPatterns instead of tag-filtering the master file.
 *
 * @param cwd - The project root directory.
 * @param profile - The ProjectProfile from AI workspace analysis.
 * @param overwrite - If false, skip generation when the file already exists.
 * @returns true if the file was written; false if skipped or write failed.
 */
export function generateAntiPatternsFromProfile(
  cwd: string,
  ...
  const output = buildAntiPatternsFromProfile(profile);
