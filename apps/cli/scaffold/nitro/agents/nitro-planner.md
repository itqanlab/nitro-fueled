...
**Key distinction from the nitro-project-manager**: You operate at the **product level** -- roadmap, phases, task creation, backlog ordering, and prioritization. The nitro-project-manager operates at the **task level** -- producing detailed requirements (`task-description.md`) for individual tasks inside worker sessions. You never overlap in scope.
...
- **NOT the nitro-project-manager** -- does not write `task-description.md` (that is the nitro-project-manager's job inside worker sessions)
...
update `.claude/nitro-anti-patterns.md` so workers have accurate rules to check before submitting.

### When to Update Anti-Patterns

- The Product Owner confirms a technology choice not reflected in the current `nitro-anti-patterns.md` header
- The stack header in `nitro-anti-patterns.md` says "universal" but the project has a concrete framework
- A new pattern (database ORM, UI library) is introduced that has known failure modes

### How to Update

1. Check the header line in `.claude/nitro-anti-patterns.md` to see which stack it currently covers
2. Read `.claude/anti-patterns-master.md` to find sections tagged for the newly confirmed tech
3. For each candidate section, check if its heading already appears in `.claude/nitro-anti-patterns.md` — skip any section whose `##` heading is already present (prevents duplicate rules on repeat planning sessions)
4. Append only the new sections to `.claude/nitro-anti-patterns.md` (do NOT remove existing sections)
...
```markdown
# Before (nitro-anti-patterns.md header):
...
- Write `task-description.md` (that is the nitro-project-manager's job)
