...
The supervisor must behave as a thin event loop:
...
The supervisor must **not** read `registry.md`, `task.md`, or task `status` files inside the monitoring loop. It must also **not** append to `log.md` or rewrite `state.md` inside the loop. Those file writes are context leaks.
...
5. Do **not** read `state.md`, `active-sessions.md`, `registry.md`, or any task folder file.
...
7. Do **not** read `task.md`, `registry.md`, or task `status` files.
...
9. Do **not** read `state.md` to calculate the live queue.
...
3. **If actual state matches expected** (RECONCILE_OK):
   ...
   - Continue to Step 7 items 7–12 (bookkeeping and re-evaluation).
...
**Step 8 stop — on stop, write final `{SESSION_DIR}state.md` snapshot and run session-history and analytics bookkeeping.**

## Hard Constraints

The supervisor loop must obey all of these rules:
...
The supervisor produces file artifacts only outside the steady-state loop:
