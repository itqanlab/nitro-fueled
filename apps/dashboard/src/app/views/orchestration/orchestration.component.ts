...
export class OrchestrationComponent {
  ...
  public getPhaseColor(phase: OrchestrationFlowPhase): string {
    ...
    if (name.includes('team-leader') || name.includes('developer') || name.includes('devops')) return '#10b981';
  ...
  public handleFilterChange(event: Event): void {
    this.setTaskTypeFilter((event.target as HTMLSelectElement).value);
  }

  public handleCloneNameChange(event: Event): void {
    this.cloneFlowName.set((event.target as HTMLInputElement).value);
  }
