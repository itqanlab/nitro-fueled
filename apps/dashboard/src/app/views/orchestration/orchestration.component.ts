...
export class OrchestrationComponent {
  ...
  public readonly cloneStatus = signal<'idle' | 'loading' | 'success' | 'error'>('error' |  public readonly taskTypeFilter = signal<string>('ALL');

  public readonly selectedFlow = computed<OrchestrationFlow | null>(() => {
    const id = this.selectedFlowId();
    if (!id) return list[0] ?? null;  return list.find(f => f.id === id) ?? list[0] ?? null;  }
