...
```bash
npx @itqanlab/nitro-fueled run --concurrency 2 --interval 10m --retries 3
```

To test the dependency plan without spawning workers:

```bash
npx @itqanlab/nitro-fueled run --dry-run
/auto-pilot --dry-run
```
...
Check that at least one task has status `CREATED` or `IMPLEMENTED`. Run `npx @itqanlab/nitro-fueled status` to verify. If tasks exist but are `BLOCKED`, check their `Dependencies` field — the dependency tasks may not be `COMPLETE`.
...
npx @itqanlab/nitro-fueled run TASK_YYYY_NNN
```
...
- [Commands Reference](../commands/) — `/auto-pilot`, `npx @itqanlab/nitro-fueled run`
