...
- **Node.js 18+** — required for the `npx @itqanlab/nitro-fueled` CLI
- **iTerm2 (macOS)** — workers open in iTerm2 tabs; iTerm2 must be running when you start Auto-Pilot
- **nitro-cortex MCP server** — the standalone Node.js process that spawns and monitors worker sessions (see [Installation](installation/) for setup)

:::note[MCP Server Dependency]
The `nitro-cortex` MCP server lives in a separate repository. It is **required for Auto-Pilot** (`/auto-pilot`, `npx @itqanlab/nitro-fueled run`) because those features spawn workers in separate iTerm2 tabs and need the MCP server to manage those sessions.
...
|------|-----------------|
| [Installation](installation/) | Run `npx @itqanlab/nitro-fueled init`, configure the MCP server, verify setup |
