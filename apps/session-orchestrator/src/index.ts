...
server.tool(
  ...
  'Emit a phase-transition event from a worker. Enqueued into the supervisor event queue. Retrieve via get_pending_events.',
  {
    ...
    data: z.record(z.string().max(64), z.string().max(512)).optional().describe('Optional key-value payload attached to the event'),
  },
  async ({ worker_id, label, data }) => {
    // Sanitize fields written to log to prevent log injection
    const safeId = worker_id.replace(/[\r\n\x1b]/g, '<LF>');
    const safeLabel = label.replace(/[\r\n\x1b]/g, '<LF>');

    // Validate worker_id format — only accept IDs generated by spawn_worker
    if (!WORKER_ID_RE.test(worker_id)) {
      process.stderr.write(`[emit_event] rejected event: invalid worker_id format "${safeId}"\n`);
      return {
        content: [{ type: 'text' as const, text: JSON.stringify({ ok: false, error: 'invalid worker_id format' }) }],
        isError: true,
      };
    }

    // Bound payload size to prevent memory exhaustion
    if (data !== undefined && JSON.stringify(data).length > DATA_PAYLOAD_MAX_BYTES) {
      process.stderr.write(`[emit_event] rejected event: data payload exceeds ${DATA_PAYLOAD_MAX_BYTES} bytes for ${safeId}\n`);
      return {
        content: [{ type: 'text' as const, text: JSON.stringify({ ok: false, error: 'data payload too large' }) }],
        isError: true,
      };
    }
    ...
    const w = registry.get(worker_id);
    if (!w) {
      process.stderr.write(`[emit_event] worker ${safeId} not found in registry — event queued anyway\n`);
    }
