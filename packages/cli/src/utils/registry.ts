  ...
  for (const taskId of taskDirs) {
    ...
    if (existsSync(statusFilePath)) {
      try {
        ...
        process.stderr.write(`[warn] ${taskId}: could not read status file, reading state from registry.md\n`);
        status = existingById.get(taskId)?.status ?? 'CREATED';
      }
    } else {
      process.stderr.write(`[warn] ${taskId}: status file missing, reading state from registry.md\n`);
      status = existingById.get(taskId)?.status ?? 'CREATED';
    }
