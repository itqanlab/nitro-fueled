...
If `npx nitro-fueled init` fails with a permissions error, verify that the current user has write access to the project directory. On macOS, you may need to run `xcode-select --install` if native build tools are missing.
