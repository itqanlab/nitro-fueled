import type { ChildProcess } from 'node:child_process';
import { spawnTrackedProcess, killTrackedProcess } from './process-launcher.js';

export interface OpenCodeLaunchOptions {
  prompt: string;
  workingDirectory: string;
  label: string;
  model: string;
  onMessage?: (msg: Record<string, unknown>) => void;
}

export interface OpenCodeLaunchResult {
  pid: number;
  process: ChildProcess;
  logPath: string;
}

// Track exit codes to distinguish successful completion from process failure
const exitCodes = new Map<number, number>();

export function launchWithOpenCode(opts: OpenCodeLaunchOptions): OpenCodeLaunchResult {
  const args = ['run', '--model', opts.model, '--format', 'json', opts.prompt];

  return spawnTrackedProcess({
    binary: 'opencode',
    args,
    workingDirectory: opts.workingDirectory,
    label: opts.label,
    onMessage: opts.onMessage,
    onExit: (code, _signal, pid) => {
      if (code !== null && pid !== 0) {
        exitCodes.set(pid, code);
      }
    },
  });
}

/**
 * Returns the exit code of a completed opencode worker, or null if still running
 * or exit code was not captured. A non-zero exit code indicates failure.
 */
export function getOpenCodeExitCode(pid: number): number | null {
  return exitCodes.get(pid) ?? null;
}

export function killOpenCodeProcess(pid: number): boolean {
  return killTrackedProcess(pid);
}
