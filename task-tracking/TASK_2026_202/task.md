...
- packages/mcp-cortex/src/db/migrations/ (new migration — add drain_requested to sessions)
- apps/dashboard-api/src/sessions/sessions.controller.ts (modified — add PATCH /stop endpoint)
- apps/dashboard-api/src/auto-pilot/auto-pilot.service.ts (modified — check drain_requested before spawn)
- apps/dashboard/src/app/views/sessions/session-detail.component.ts (modified — End Session button + drain state)
- apps/dashboard/src/app/views/sessions/session-detail.component.html (modified — drain UI)
- apps/dashboard/src/app/services/api.service.ts (modified — add stopSession() call)
- apps/dashboard/src/app/models/api.types.ts (modified — add drain_requested to Session type)
